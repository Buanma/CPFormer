...
def get_indices_params(xyz, batch, window_size, shift_win: bool):
    if isinstance(window_size, list) or isinstance(window_size, np.ndarray):
        ...
        window_size = torch.tensor([window_size] * 3).type_as(xyz).to(xyz.device)  # 将window_size转为和xyz同数据类型和设备的张量

    if shift_win:
        v2p_map, k, counts = grid_sample(xyz + 1 / 2 * window_size, batch, window_size, start=xyz.min(0)[0],
                                         return_p2v=False, return_counts=True)#是否进行窗口偏移
    else:
        v2p_map, k, counts = grid_sample(xyz, batch, window_size, start=xyz.min(0)[0], return_p2v=False,
                                         return_counts=True)
    v2p_map, sort_idx = v2p_map.sort() #排序并返回索引
...
def tri_plane_self_attention(
    ...
    attn_flat = dot_prod_with_idx_all_tri_plane(query, index_0, index_0_offsets, key, index_1, index_1_offsets,
                                                relative_pos_query_table, relative_pos_key_table,
                                                relative_position_index, n_max, m_offsets)
    index_0_offset_shape = query.shape[0] + 1
    softmax_attn_flat = torch.zeros_like(attn_flat)
    softmax_attn_flat[0:m_offsets[0]] = scatter_softmax_csr(src=attn_flat[0:m_offsets[0]],
                                                            indptr=index_0_offsets[0:index_0_offset_shape].long(),
                                                            dim=0)
    softmax_attn_flat[m_offsets[0]:m_offsets[1]] = scatter_softmax_csr(src=attn_flat[m_offsets[0]:m_offsets[1]],
                                                                       indptr=index_0_offsets[
                                                                              index_0_offset_shape:2 * index_0_offset_shape].long(),
                                                                       dim=0)
    softmax_attn_flat[m_offsets[1]:m_offsets[2]] = scatter_softmax_csr(src=attn_flat[m_offsets[1]:m_offsets[2]],
                                                                       indptr=index_0_offsets[
                                                                              2 * index_0_offset_shape:].long(), dim=0)
    x = attention_step2_with_rel_pos_value_tri_plane(softmax_attn_flat, value, index_0, index_0_offsets, n_max, index_1,
                                                     index_1_offsets, relative_pos_value_table, relative_position_index,
                                                     m_offsets)
    return x
...
def sparse_self_attention(query,
    ...
    attn_flat = dot_prod_with_idx_all(query, index_0, index_0_offsets, key, index_1, index_1_offsets,
                                      relative_pos_query_table, relative_pos_key_table, relative_position_index, n_max)

    softmax_attn_flat = scatter_softmax_csr(src=attn_flat, indptr=index_0_offsets.long(), dim=0)  # [M, num_heads]
    x = attention_step2_with_rel_pos_value(softmax_attn_flat, value, index_0, index_0_offsets, n_max, index_1,
                                           index_1_offsets, relative_pos_value_table, relative_position_index)
